...
def cpp_minimize(
    ...
    solve_U: Any | None = None,
    **kwargs: Any,
) -> tuple[Any, Any, dict[str, Any]]:
    """Run minimization using the lazily loaded C++ backend.

    Args:
        m: Nodal magnetization array.
        B_ext: Applied field vector.
        U0: Initial scalar potential.
        params: Minimizer parameter object.
        sparse_ops: Assembled SciPy sparse matrices.
        solve_U: Optional Poisson solver carrying a PARDISO owner.
        **kwargs: Optional boundary mask and minimizer values.

    Returns:
        Updated magnetization, scalar potential, and minimizer diagnostics.
    """
    from ._native_loader import require_cpp_minimizer

    minimizer_function = require_cpp_minimizer()
    N = U0.shape[0]
    ...
    handle_context = nullcontext(0)
    if solve_U is not None and hasattr(solve_U, "pardiso_obj"):
        handle_context = solve_U.pardiso_obj.borrow_handle_id()
    ...
    with handle_context as pardiso_handle:
        native_status = minimizer_function(
            N,
        ...
        if native_status != 0:
            raise RuntimeError(f"C++ minimizer failed with native status {native_status}")
