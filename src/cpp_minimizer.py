...
lib.run_cpp_pcohen_hs_minimization.argtypes = [
    ...
    ctypes.c_double, ctypes.c_double, ctypes.c_int, ctypes.c_double, ctypes.c_int64,
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), # m, B_ext, U, M_nodal
    ...
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int),
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int),
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), # inv_M_rel, inv_M_prec
    ctypes.c_int, ctypes.c_double, ctypes.c_double, # pc_auto, pc_force_eta, pc_force_alpha
    ctypes.c_double, ctypes.c_int, ctypes.c_double, ctypes.c_double, ctypes.c_int, ctypes.c_int, # cg_tol, pc_iters, pc_reg, pc_stagnation_nu, L, beta_type
    ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), # out_iters, out_evals, out_demag, out_preco
...
def cpp_minimize(m, B_ext, U0, params, sparse_ops, solve_U=None, **kwargs):
    ...
    lib.run_cpp_pcohen_hs_minimization(
        N, getattr(params, 'max_iter', 2000), getattr(params, 'tau_f', 1e-8),
        getattr(params, 'eps_a', kwargs.get('eps_a', 1e-6)),
        getattr(params, 'ls_eta1', 1e-4), getattr(params, 'ls_eta2', 0.9),
        getattr(params, 'ls_C', 2.0), getattr(params, 'ls_c', 0.5),
        getattr(params, 'ls_max_evals', 15), inv_Vmag, pardiso_handle,
        ...
        pc_auto_val, pc_force_eta_val, pc_force_alpha_val,
        getattr(params, 'pc_tol', 0.01), getattr(params, 'pc_iters', 10), getattr(params, 'pc_reg', 0.0),
        getattr(params, 'pc_stagnation_nu', 1e-3),
        params.L if getattr(params, 'L', None) is not None else N,
        beta_type_val,
        ctypes.byref(out_iters), ctypes.byref(out_evals), ctypes.byref(out_demag), ctypes.byref(out_preco), ctypes.byref(out_E), ctypes.byref(out_gnorm)
    )
