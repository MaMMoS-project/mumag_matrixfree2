...
def plot_demagnetization_curves(all_data, data_avg, props, out_dir, eval_name, Neff=1.0/3.0):
    ...
    rcParams.update({'font.size': 14, 'axes.labelsize': 16, 'legend.fontsize': 12, 'xtick.labelsize': 14, 'ytick.labelsize': 14})
    ...
    if J_ext_left > 0: axs[0].plot([-Bc_ext, B_ext_left], [0, J_ext_left], 'r--')
    ...
    if J_int_left > 0: axs[1].plot([-Bc_int, B_int_left], [0, J_int_left], 'r--')
    ...
    if M_int_left > 0: axs[2].plot([-Hc_int, H_int_left], [0, M_int_left], 'r--')
...
def main():
    ...
    if indiv_csv.exists() and os.path.getsize(indiv_csv) > 0:
        try: c_indiv = me.from_csv(indiv_csv)
        except Exception: c_indiv = me.EntityCollection()
    else:
        c_indiv = me.EntityCollection()

    if summary_csv.exists() and os.path.getsize(summary_csv) > 0:
        try: c_summary = me.from_csv(summary_csv)
        except Exception: c_summary = me.EntityCollection()
    else:
    ...
    for struct_dir in struct_dirs:
        ...
        if mh_file.exists():
            ...
            c_indiv["CoercivityHcExternal"] = append_val(c_indiv, "CoercivityHcExternal", Hc_ext_Am, lambda v: me.Entity("CoercivityHcExternal", v, "A/m"))
    ...
    if all_data:
        ...
        try:
            ...
            c_summary["CoercivityHcExternal"] = append_val(c_summary, "CoercivityHcExternal", Hc_ext_Am, lambda v: me.Entity("CoercivityHcExternal", v, "A/m"))
            ...
            c_summary["CoercivityHc"] = append_val(c_summary, "CoercivityHc", Hc_int, lambda v: me.Entity("CoercivityHc", v, "A/m"))
            c_summary["Remanence"] = append_val(c_summary, "Remanence", Mr_int, lambda v: me.Entity("Remanence", v, "A/m"))
