...
def run_sw_me_test(inp_path: str, phi_deg: float = 0.0):
    ...
    k1me = jnp.asarray(k1me_arr)
    k1me_p = jnp.asarray(k1mep_arr)
    ...
    minimize = make_minimizer_no_demag(
        geom, A_lookup, K1_lookup, Js_lookup, k_easy_lookup, V_mag, M_nodal, k1me, k1me_p
    )
    ...
    for theta_deg in angles_deg:
        ...
        h_dir = np.array([np.sin(theta_rad) * np.cos(phi_rad), np.sin(theta_rad) * np.sin(phi_rad), np.cos(theta_rad)])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inp",
        type=str,
        default="lsdyna/ring.mappend.inp",
        help="Path to the input AVS UCD (.inp) mesh file containing 'k1me' and 'k1me_p' cell data.",
    )
    parser.add_argument(
        "--phi", type=float, default=0.0, help="Azimuthal angle of the field rotation plane (degrees, 0=XZ, 90=YZ)."
    )
