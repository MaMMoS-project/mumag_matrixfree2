...
\begin{equation}
    e_{\text{uniax, num}} = \frac{1}{2} \mathbf{m} \cdot \left( -2 K_1 (\mathbf{m}_i \cdot \mathbf{e}_z) \mathbf{e}_z \right) = -K_1 m_z^2
\end{equation}
This is related to the standard form $e = K_1(1 - m_z^2)$ by a constant offset $-K_1$.

\subsection{Orthorhombic Anisotropy ($K_{1p}, k_{1\text{me}}$)}
The orthorhombic energy density in the local crystal frame $(\mathbf{e}_x, \mathbf{e}_y, \mathbf{e}_z)$ is:
\begin{equation}
    e_{\text{ortho}} = K_x m_x^2 + K_y m_y^2
\end{equation}
where $K_x$ and $K_y$ combine material-level and per-element contributions:
\begin{align}
    K_x^{(e)} &= K_{1p} + (k_{1\text{me}}^{(e)} + k'_{1\text{me}, e}) \\
    K_y^{(e)} &= -K_{1p} + (k_{1\text{me}}^{(e)} - k'_{1\text{me}, e})
\end{align}
The total numerical energy for multiple elements is the volume-weighted average:
\begin{equation}
    E_{\text{ortho, tot}} = \sum_e (K_x^{(e)} m_x^2 + K_y^{(e)} m_y^2) V_e
\end{equation}
...
\subsection{Per-Element Anisotropy Case}
Two elements with volumes $V_0, V_1$ and identity crystal orientation.
\begin{itemize}
    \item Element 0: $k_{1\text{me}}=1.0, k'_{1\text{me}}=0.5 \implies K_x=1.5, K_y=0.5$.
    \item Element 1: $k_{1\text{me}}=2.0, k'_{1\text{me}}=1.0 \implies K_x=3.0, K_y=1.0$.
\end{itemize}
For uniform $\mathbf{m}=(1,0,0)$, expected total energy density:
\begin{equation}
    e = \frac{1.5 V_0 + 3.0 V_1}{V_0 + V_1}
\end{equation}
Result: \textbf{Passed}.
