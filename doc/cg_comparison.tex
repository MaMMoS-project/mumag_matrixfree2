...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \midrule
        \texttt{wg\_np}             & 28.307 & 221 & 322 & 0 & 3481 \\
        \texttt{tr}               & \textbf{10.373} & 119 & 119 & 431 & \textbf{1696} \\
        \texttt{ptr}              & 22.566 & \textbf{66} & \textbf{66} & 795 & 7961 \\
        \texttt{tn\_split}        & 13.477 & 159 & 246 & 568 & 3522 \\
        \texttt{pcg}              & 19.441 & 103 & 556 & 394 & 5552 \\
        \texttt{pcohen}           & 12.047 & 142 & 214 & 582 & 2738 \\
        \texttt{pcohen\_exact}    & 14.202 & 144 & 204 & 586 & 2549 \\
        \texttt{pcohen\_hs}       & 12.384 & 161 & 191 & 684 & 2498 \\
        \texttt{pcohen\_hs\_exact}& 12.075 & 164 & 204 & 692 & 2738 \\
        \texttt{pcohen\_lbfgs}     & 11.959 & 187 & 278 & 0 & 3641 \\
        ...
        \texttt{lbfgs} ($m=3$)     & 11.366 & 177 & 270 & 0 & 3362 \\
        \texttt{lbfgs} ($m=5$)     & 13.086 & 177 & 270 & 0 & 3362 \\
        \texttt{lbfgs} ($m=10$)    & 11.795 & 177 & 270 & 0 & 3308 \\
        \texttt{plbfgs} ($m=3$)    & 14.386 & 98 & 228 & 98 & 3035 \\
        \texttt{plbfgs} ($m=5$)    & 14.247 & 98 & 228 & 98 & 3036 \\
        \texttt{plbfgs} ($m=10$)   & 13.983 & 98 & 228 & 98 & 3036 \\
        \texttt{dplbfgs} ($m=3$)   & 12.766 & 88 & 194 & 176 & 2426 \\
        \texttt{dplbfgs} ($m=5$)   & 13.883 & 88 & 194 & 176 & 2427 \\
        \texttt{dplbfgs} ($m=10$)  & 12.853 & 88 & 194 & 176 & 2387 \\
        \texttt{pcohen\_lbfgs} ($m=3$)  & 12.710 & 187 & 278 & 0 & 3599 \\
        \texttt{pcohen\_lbfgs} ($m=5$)  & 11.959 & 187 & 278 & 0 & 3641 \\
        \texttt{pcohen\_lbfgs} ($m=10$) & 12.352 & 187 & 278 & 0 & 3643 \\
...
\subsection{The Necessity of Manifold Transport}
The standard \texttt{pcg} algorithm demonstrates the catastrophic consequences of ignoring manifold curvature. Although it requires the fewest outer iterations (103), the proposed search directions are consistently poor, forcing the Armijo line search to reject steps repeatedly. This causes an explosion in function evaluations ($N_f = 556$) and inner Poisson solves ($N_{\text{demag}} = 5552$).
Cohen's method immediately resolves this issue. By projecting the search directions onto the tangent space, it ensures conjugacy is maintained on the spherical manifold. Comparing the $\beta$ update strategies, the Hestenes-Stiefel variants (\texttt{\_hs}) propose higher-quality descent directions than Polak-Ribière, reducing the total function evaluations and Poisson solves. Interestingly, while the rigorous exact Cayley transport (\texttt{pcohen\_exact}) provides a minor speedup for the Polak-Ribière variant, it degrades performance slightly for the Hestenes-Stiefel variant. The computationally cheaper tangent projection is more than sufficient for numerical accuracy.

\subsection{Preconditioning and Secant Consistency}
Our testing of the unpreconditioned Wen-Goldfarb algorithm (\texttt{wg\_np}) yielded surprisingly competitive metrics for an un-smoothed solver (28.307 s, 221 outer iterations). While this is slower than the top-tier Trust Region methods, its strict adherence to the BB spectral step sizes allows it to significantly outperform standard gradient descent without requiring any internal preconditioner. This highlights the inherent scale-invariant efficiency of the Barzilai-Borwein formulation when applied strictly along the exact tangent space descent direction.
Crucially, Barzilai-Borwein relies on the historical consistency of the secant equation ($\langle \mathbf{s}, \mathbf{y} \rangle$) to build a 1D approximation of the inverse Hessian. If a spatial preconditioner $P(\mathbf{m})$ is applied that changes at every step, this historical consistency is destroyed, which is why BB methods must remain unpreconditioned in this framework. Conversely, L-BFGS (\texttt{plbfgs}) thrives with a preconditioner because the AMG smoother $P^{-1}$ acts as the initial inverse Hessian approximation $H_0$ within the two-loop recursion, naturally mapping the extensive gradient into a preconditioned intensive vector field while satisfying metric consistency.

\subsection{Momentum vs. Adaptive Acceleration}
The massive performance gap between Preconditioned Nesterov Accelerated Gradient (\texttt{pnag}: 42.7 s, 397 iterations) and Anderson Acceleration (\texttt{aapg}: 16.4 s, 173 iterations) illustrates how different algorithms react to the extreme stiffness of the micromagnetic energy landscape.
Nesterov momentum uses a hardcoded, blind push forward. In the narrow, twisting valleys of micromagnetics, this blind momentum constantly overshoots and crashes into the steep walls of the energy landscape, forcing the Armijo line search to repeatedly slash the step size (resulting in $N_f = 1103$ evaluations). Furthermore, Nesterov momentum assumes a flat Euclidean space; evaluating the gradient at an off-manifold look-ahead point introduces severe geometric distortion.
In contrast, Anderson Acceleration (\texttt{aapg}) \cite{Anderson1965Iterative} is entirely data-driven. It performs a least-squares optimization over the history of the last $m$ residuals. If it detects that the trajectory is oscillating or overshooting, the least-squares solver naturally outputs coefficients that cancel out those oscillations, dynamically dampening itself.

\subsection{The Superiority of Trust Region Methods}
While the history-based variants (\texttt{plbfgs}, \texttt{dplbfgs}, and \texttt{pcohen\_lbfgs}) demonstrate astonishing algorithmic efficiency, the newly implemented Local Trust Region algorithm (\texttt{tr}) completely eclipses all other line-search based methods, setting an unprecedented speed record of just $10.373$ s. By swapping out the full Hessian constraint with a localized Steihaug-Toint boundary approach, \texttt{tr} drives the outer iterations down to a mere $119$. Because it avoids line-search evaluations entirely, the number of inner Poisson solves plummets to an absolute minimum of $1696$.
The Preconditioned Trust Region (\texttt{ptr}) variant demonstrates a fascinating mathematical dichotomy. It yields incredibly accurate true-Newtonian search directions, completing the entire hysteresis loop in an astonishingly minimal \textbf{66 outer steps}. However, resolving the full Hessian requires an expensive Poisson solve during every single inner Steihaug iteration. The total demagnetization solver count explodes to $7961$, proving that while exact non-local Newtonian trajectories are geometrically optimal, the highly matrix-free, locally-approximated \texttt{tr} method is computationally superior.

\subsection{Influence of L-BFGS Memory Size}

A parametric sweep of the L-BFGS memory capacity ($m=3$, $m=5$, and $m=10$) reveals a remarkably profound physical result: the number of outer iterations ($N_{\text{iter}}$) and function evaluations ($N_f$) are almost entirely invariant to the memory size. For instance, \texttt{dplbfgs} strictly requires $88$ outer iterations regardless of whether $m=3$, $m=5$, or $m=10$. This invariance is a direct testament to the extreme efficiency of the spatial preconditioners and the Newton-like trajectory of the solver.

This phenomenon can be deduced mathematically from the structure of the simulation. The hysteresis loop evaluates the external magnetic field from $+2.0$ T down to $-6.0$ T in decrements of $-0.5$ T, resulting in exactly $17$ distinct field steps. Taking the $88$ total iterations required by \texttt{dplbfgs} and dividing by the $17$ field steps yields an average of merely $5.17$ outer iterations per step. Because the optimizer completely resets and flushes the L-BFGS history arrays to zero at the beginning of each external field step, the history buffer only accumulates roughly $5$ vectors before the optimizer successfully hits the energy convergence tolerance. 

Consequently, a memory buffer of size $m=10$ provides zero algorithmic benefit because the optimizer physically reaches the minimum before it can even reach index 6. Even for a smaller buffer of $m=3$, the algorithm only overwrites the 4th and 5th oldest vectors. Because these older vectors are too stale to significantly improve the local secant approximation, losing them has absolutely zero impact on the required number of steps, locking the overall convergence trajectory in place.

\section{Conclusion}
This report has established rigorous foundations for optimization on the micromagnetic manifold, spanning from Conjugate Gradient and Limited-Memory quasi-Newton methods to Trust Region and Barzilai-Borwein curvilinear descents. The culmination of this benchmarking suite definitively proves that incorporating local spatial stiffness through a preconditioned Steihaug-Toint \cite{steihaug1983conjugate} Trust Region boundary yields unparalleled computational efficiency, while exact manifold transport is absolutely crucial for L-BFGS convergence. Future work will investigate the coupling of these highly efficient matrix-free minimizers with dynamic stochastic thermal noise models and generalized semi-implicit integration schemes.

\bibliographystyle{plain}
\bibliography{references}

\end{document}\section{Conclusion}
This report has established rigorous foundations for optimization on the micromagnetic manifold, spanning from Conjugate Gradient and Limited-Memory quasi-Newton methods to Trust Region and Barzilai-Borwein curvilinear descents. The culmination of this benchmarking suite definitively proves that incorporating local spatial stiffness through a preconditioned Steihaug-Toint Trust Region boundary yields unparalleled computational efficiency, while exact manifold transport is absolutely crucial for L-BFGS convergence. Future work will investigate the coupling of these highly efficient matrix-free minimizers with dynamic stochastic thermal noise models and generalized semi-implicit integration schemes.
