...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \texttt{pcohen\_hs}       & 12.384 & 161 & \textbf{191} & 684 & 2498 \\
        \texttt{pcohen\_hs\_exact}& 12.075 & 164 & 204 & 692 & 2738 \\
        \texttt{pcohen\_lbfgs}     & \textbf{11.959} & 187 & 278 & 0 & 3641 \\
        ...
        \texttt{dplbfgs} ($m=10$)  & 12.853 & \textbf{88} & 194 & 176 & \textbf{2387} \\
        \texttt{pcohen\_lbfgs} ($m=3$)  & 12.710 & 187 & 278 & 0 & 3599 \\
        \texttt{pcohen\_lbfgs} ($m=5$)  & \textbf{11.959} & 187 & 278 & 0 & 3641 \\
...
Conversely, the ultimate speed record goes to the hybrid \texttt{pcohen\_lbfgs} ($m=5$). By discarding the expensive AMG spatial smoother entirely ($N_{\text{preco}} = 0$) and relying solely on the parallel-transported L-BFGS history arrays to provide the preconditioner for the Cohen CG update, the algorithm achieves an unmatched wall-clock time of $11.959$ s. While it naturally requires slightly more outer iterations (187) and Poisson solves (3641) than \texttt{dplbfgs} due to the lack of an explicit spatial PDE smoother, its pure-GPU execution model shreds through the computational overhead, cementing it as the absolute fastest minimizer in the framework.
