...
The Wen-Goldfarb algorithm (\texttt{wg\_np}) is a pure curvilinear search method utilizing Barzilai-Borwein (BB) spectral step sizes. Unlike preconditioned methods, it relies solely on the raw tangent gradient. The algorithm strictly bounds the step size and alternates between two BB estimators ($\tau_1$ and $\tau_2$) to dynamically capture the local inverse Rayleigh quotient. Because the BB step lengths inherently construct a scale-invariant secant approximation over consecutive steps, applying a dynamic spatial preconditioner $P(\mathbf{m})$ mathematically destroys the required historical consistency. Thus, this variant completely drops the preconditioner, allowing the alternating BB steps to blindly traverse the stiff spherical manifold efficiently, switching to pure BB steps after two initial Armijo line search steps.
