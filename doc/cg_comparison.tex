...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \texttt{dplbfgs}           & 13.883 & \textbf{88} & 194 & 176 & \textbf{2427} \\
...
Conversely, the ultimate speed record goes to the hybrid \texttt{pcohen\_lbfgs}. By discarding the expensive AMG spatial smoother entirely ($N_{\text{preco}} = 0$) and relying solely on the parallel-transported L-BFGS history arrays to provide the preconditioner for the Cohen CG update, the algorithm achieves an unmatched wall-clock time of $11.959$ s. While it naturally requires slightly more outer iterations (187) and Poisson solves (3641) than \texttt{dplbfgs} due to the lack of an explicit spatial PDE smoother, its pure-GPU execution model shreds through the computational overhead, cementing it as the absolute fastest minimizer in the framework.
