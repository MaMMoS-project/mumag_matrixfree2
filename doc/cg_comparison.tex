...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \texttt{ptr}              & 22.710 & \textbf{66} & \textbf{66} & 795 & 7970 \\
...
In contrast, the Preconditioned Trust Region (\texttt{ptr}) variant demonstrates a fascinating mathematical dichotomy. By explicitly coupling the non-local full Hessian operator with the local spatial AMG preconditioner, \texttt{ptr} yields incredibly accurate true-Newtonian search directions. It slashes through the complex manifold landscape to complete the entire hysteresis loop in an astonishingly minimal \textbf{66 outer steps}, setting the absolute record for shortest geometric trajectory. However, because resolving the full Hessian requires an expensive Poisson solve during every single inner Steihaug iteration, the total demagnetization solver count explodes to $7970$, driving its wall-clock time to $22.710$ s. This ultimately proves that while exact non-local Newtonian trajectories are geometrically optimal, the highly matrix-free, locally-approximated \texttt{tr} method is computationally superior for high-performance micromagnetics.
