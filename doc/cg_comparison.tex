...
A parametric sweep of the L-BFGS memory capacity ($m=3$, $m=5$, and $m=10$) reveals a remarkably profound physical result: the number of outer iterations ($N_{\text{iter}}$) and function evaluations ($N_f$) are almost entirely invariant to the memory size. For instance, \texttt{dplbfgs} strictly requires $88$ outer iterations regardless of whether $m=3$ or $m=10$. This invariance is a direct testament to the extreme efficiency of the spatial preconditioners and the Newton-like trajectory of the solver. During the tracing of the magnetic hysteresis loop, the algorithm converges the state of the system for each external field step in an average of merely $4$ to $5$ iterations. Because the history arrays are cleared between external field steps, the optimizer reaches the minimum before a memory buffer of size $m>4$ can even be fully populated. Consequently, a memory size of $m=5$ is completely optimal for micromagnetic hysteresis tracking, and larger buffers provide zero algorithmic benefit while slightly altering memory transfer timings.
