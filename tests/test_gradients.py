...
def setup_geom():
    ...
    return {
        'geom': geom, 'A_lookup': A_lookup, 'K1_lookup': K1_lookup, 
        'Js_lookup': Js_lookup, 'k_easy_lookup': k_easy_lookup, 
        'V_mag_nm': V_mag_nm, 'M_nodal': M_nodal, 'boundary_mask': boundary_mask,
        'knt': knt, 'Js_si': Js
    }

def check_gradient_consistency(geom, a_l, k1_l, js_l, k_easy_l, v_mag, m_nodal, solve_U, m_nodes, b_ext, name):
    _, energy_fn, grad_fn = make_energy_kernels(geom, a_l, k1_l, js_l, k_easy_l, v_mag, m_nodal)
    
    m0 = jnp.asarray(m_nodes)
    u0 = solve_U(m0, jnp.zeros(m0.shape[0])) if name == "DEMAG" else jnp.zeros(m0.shape[0])
...
def test_exchange_gradient(setup_geom):
    d = setup_geom
    m_hel = np.column_stack([np.cos(0.1 * d['knt'][:, 0]), np.sin(0.1 * d['knt'][:, 0]), np.zeros(d['knt'].shape[0])])
    solve_U = make_solve_U(d['geom'], d['Js_lookup'], cg_tol=1e-12, boundary_mask=d['boundary_mask'])
    check_gradient_consistency(d['geom'], d['A_lookup'], jnp.zeros(2), jnp.zeros(2), d['k_easy_lookup'], d['V_mag_nm'], d['M_nodal'], solve_U, m_hel, jnp.zeros(3), "EXCHANGE")

def test_anisotropy_gradient(setup_geom):
    d = setup_geom
    m_45 = np.tile(np.array([1.0, 0.0, 1.0]) / np.sqrt(2.0), (d['knt'].shape[0], 1))
    solve_U = make_solve_U(d['geom'], d['Js_lookup'], cg_tol=1e-12, boundary_mask=d['boundary_mask'])
    check_gradient_consistency(d['geom'], jnp.zeros(2), d['K1_lookup'], jnp.zeros(2), d['k_easy_lookup'], d['V_mag_nm'], d['M_nodal'], solve_U, m_45, jnp.zeros(3), "ANISOTROPY")
...
def test_zeeman_gradient(setup_geom):
    ...
    solve_U = make_solve_U(d['geom'], d['Js_lookup'], cg_tol=1e-12, boundary_mask=d['boundary_mask'])
    check_gradient_consistency(d['geom'], jnp.zeros(2), jnp.zeros(2), d['Js_lookup'], d['k_easy_lookup'], d['V_mag_nm'], d['M_nodal'], solve_U, m_x, b_ext, "ZEEMAN")

def test_demag_gradient(setup_geom):
    d = setup_geom
    m_x = np.tile(np.array([1.0, 0.0, 0.0]), (d['knt'].shape[0], 1))
    solve_U = make_solve_U(d['geom'], d['Js_lookup'], cg_tol=1e-12, boundary_mask=d['boundary_mask'])
    check_gradient_consistency(d['geom'], jnp.zeros(2), jnp.zeros(2), d['Js_lookup'], d['k_easy_lookup'], d['V_mag_nm'], d['M_nodal'], solve_U, m_x, jnp.zeros(3), "DEMAG")
