...
def test_micromagnetic_energies():
    ...
    knt, ijk = add_shell.run_add_shell_pipeline(
        in_npz=str(tmp_path), layers=8, K=1.4, h0=h, verbose=False
    )
    ...
    # Exchange test
    _, E_only_ex, _ = make_energy_kernels(
        geom,
        A_lookup,
        K_zero,
        J_zero,
    ...
    _, E_only_z, _ = make_energy_kernels(
        geom,
        A_zero,
        K_zero,
        Js_lookup,
    ...
    e_z_si = (
        float(
            E_only_z(
                m_unif_x, jnp.zeros_like(U_jax), jnp.asarray([B_ext_si / Js, 0, 0])
            )
        )
        * SI_FACTOR
    )
    ...
    _, E_only_d, _ = make_energy_kernels(
        geom,
        A_zero,
        K_zero,
        Js_lookup,
    ...
    _, E_only_an, _ = make_energy_kernels(
        geom,
        A_zero,
        K1_lookup,
        J_zero,
